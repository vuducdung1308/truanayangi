...
export default function Home(){
 ...
 <Dialog open={revealed} onOpenChange={setRevealed}><DialogContent className="winner-dialog" showCloseButton={false}>{result&&<><span className="winner-label">VẬT PHẨM MỚI</span><DialogTitle className="winner-title">{result.name}</DialogTitle><DialogDescription className="winner-description">{mode==='friday'?`${result.sub} · `:'Giá tham khảo · '}~{result.price}.000đ / người <span className="price-asof">· khảo giá 09/2026</span></DialogDescription>{priorDays.length>0&&<p className="winner-history">🗓️ Đã chọn món này {priorDays.length===1?`ngày ${priorDays[0]}`:`vào ${priorDays.length} ngày: ${priorDays.slice(-4).join(' · ')}`}{priorDays.length>4?` (+${priorDays.length-4})`:''}</p>}<div className="winner-art" style={{'--rarity':colors[result.rarity]} as React.CSSProperties}><FoodImage food={result}/></div><div className="winner-actions"><a className="find-button" href={findNearbyUrl(mode==='friday'?`${result.name} ${result.sub.replace(/\s*•\s*/g,' ')}`:result.name)} target="_blank" rel="noreferrer">TÌM QUÁN <ArrowUpRight size={16}/></a><button onClick={()=>setRevealed(false)}>TIẾP TỤC</button></div></>}</DialogContent></Dialog>
